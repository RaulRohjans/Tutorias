...
{
    public partial class Dashboard_Al : Form
    {
        public Dashboard_Al()
        {
            InitializeComponent();
        ...
        private void Dashboard_Al_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (apoios[i].Estado != Apoios.enumEstado.Concluido && apoios[i].AlunoID == mAlunoID)
                    {
            ...
            {
                MessageBox.Show("Ocorreu um erro ao carregar a informação dos apoios.", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            }
        ...
        {
            Form_Pedir_ApAL PedAp = new Form_Pedir_ApAL();
            PedAp.Passa(mAlunoID);
            PedAp.Show();
        }
